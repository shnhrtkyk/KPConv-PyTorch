...
class MyDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        """
        PointCloudDataset.__init__(self, 'MyDataset')
        ...
        self.cloud_names = ['Training1', 'Training2', 'Training3', 'Training4', 'Training5']
        ...
        for i, f in enumerate(self.cloud_names):
            if self.set == 'training':
                if self.all_splits[i] != self.validation_split:
                    self.files += [join(ply_path, f + '.las')]
            elif self.set in ['validation', 'test', 'ERF']:
                if self.all_splits[i] == self.validation_split:
                    self.files += [join(ply_path, f + '.las')]
            else:
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            sub_ply_file = join(tree_path, '{:s}.las'.format(cloud_name))
            ...
            if exists(KDTree_file):
                ...
                # read ply with data
                file_path = sub_ply_file + ".ply"
                data = read_ply(file_path)
                sub_colors = np.vstack((data['red'], data['green'], data['blue'])).T
                sub_labels = data['class']
                lbl = np.unique(sub_labels[sub_labels >= 0])
                print(lbl)
                ...
                file_h = laspy.file.File(file_path, mode='r')
                # print(file_h)
                xyz = file_h.points['point']
                points = np.vstack([file_h.x, file_h.y, file_h.z]).transpose()
                # print((points.dtype))
                points = points.astype(np.float32)
                # print((points.dtype))
                attr_names = [a for a in xyz.dtype.names] + MyDataset.ATTR_EXTRA_LIST
                colors = np.array([getattr(file_h, name) for name in attr_names
                                           if name not in MyDataset.ATTR_EXLUSION_LIST]).transpose()
                labels = file_h.classification
                lbl = np.unique(labels[labels >= 0])
                print(lbl)
                # Subsample cloud
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(proj_file):
                    ...
                    points = np.vstack([file_h.x, file_h.y, file_h.z]).transpose()
                    attr_names = [a for a in xyz.dtype.names] + MyDataset.ATTR_EXTRA_LIST
                    sub_colors = np.array([getattr(file_h, name) for name in attr_names
                                           if name not in MyDataset.ATTR_EXLUSION_LIST]).transpose()
                    labels = file_h.classification
